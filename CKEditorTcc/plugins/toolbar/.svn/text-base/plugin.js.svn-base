...
(function() {
	...
	CKEDITOR.plugins.add( 'toolbar', {
		//Para ter acesso ao método getPrivateToolbarGroups()
		//TODO exite outra maneira de chamar o método sem precisar alterar esta classe?
		getToolbar : function() {
			return toolbars;
		},
		...
		init: function( editor ) {
			...
			//Criei esta variável para tornar público a lisrta de grupos e items de ferramentas
			var toolbars;
			...
			editor.on( 'uiSpace', function( event ) {
				if ( event.data.space != editor.config.toolbarLocation )//onde ficará a barra de ferramentas, top, down ...
					return;

				// Create toolbar only once.
				event.removeListener(); //event é o uiSpace	

				editor.toolbox = new toolbox();

				var labelId = CKEDITOR.tools.getNextId(); //obtem uma id úina para um elemto de interface ui.
				/**
				 * aqui é adicionado o recurso de ACESSIBILIDADE  que disponibiliza para o NVDA a frase editor.lang.toolbar.toolbars Barra de Ferramentas do Editor
				 */
				var output = [
				...
				//editor.config.toolbarStartupExpanded se true então a barra de ferramenta inicia expandida
				...
				var toolbars = editor.toolbox.toolbars, //ferramentas e seus grupos vazios, ainda não foi adicionado 
					toolbar = getToolbarConfig( editor );//busca as ferramentas e seus grupos padrão ou customizado pelo usuário
 ...
 */
				for ( var r = 0; r < toolbar.length; r++ ) {
					var toolbarId,
						toolbarObj = 0,
						toolbarName,
						row = toolbar[ r ],//row.name   row.item[0].name
						items;
					...
					if ( groupStarted ) {//flag que indica que terminou a construção do HTML podendo finalizar o span
						output.push( '</span>' );
						groupStarted = 0;
						pendingSeparator = 0;
					}
					...
					if ( row === '/' ) {
						output.push( '<span class="cke_toolbar_break"> </span>' );//se o grupo é um / então coloca o separador de grupos na interface
						continue;
					}

					items = row.items || row;//itens do grupo ex: bold, italic.... não traduzidos
					...
					for ( var i = 0; i < items.length; i++ ) {
						...
						if ( item ) {
							...
							if ( !toolbarObj ) {
								...
								toolbarName = row.name && ( editor.lang.toolbar.toolbarGroups[ row.name ] || row.name ); // pega o nome do grupo de ferramentas traduzido para o nvda ler
							...
							/**
							 * ENVIA O NOME DA BARRA DE FERRAMENTAS
							 */
								toolbarName && output.push( '<span id="', toolbarId, '_label" class="cke_voice_label">', toolbarName, '</span>' );
								output.push( '<span class="cke_toolbar_start"></span>' );//o grupo recebe o foco então o nvda lê o nome
							...
							function addItem( item ) { //item contém os commandos ex bold, italic ... não traduzido
	...
	function getToolbarConfig( editor ) {
		...
		/**
		 * Items de ferramentas dos grupos
		 * Quando as ferramentas são customizadas esta função não é executada
		 */
...
//				alert('grupo: '+(itemToolbar = item.toolbar || 'others'));
				if ( itemToolbar ) {
...
//			alert(groups.basicstyles.name)
		...
		function populateToolbarConfig( config ) {
			...
			for ( i = 0; i < config.length; ++i ) {
				group = config[ i ]; //grupos conforme configurado em config.toolbar
				newGroup = {};
				if ( group == '/' )
					toolbar.push( group );
		...
		if ( typeof toolbar == 'string' )
			toolbar = editor.config[ 'toolbar_' + toolbar ];
				...
				/*
		 * Criei esta variável global para armazenar as barras de ferramentas, pois chamando getToolbarConfig()
		 * estava criando duas barras
		 */
		
		//FUNCIONOU ASSIM, AGORA TEM QUE TENTAR EXTENDER E CHAMAR AS FUNÇÕES.
		//this.toolbars = editor.toolbar = toolbar ? populateToolbarConfig( toolbar ) : CKEDITOR.tools.clone( editor.config.toolbarGroups ) || getPrivateToolbarGroups( editor );
			

		this.toolbars = ( editor.toolbar = toolbar ? populateToolbarConfig( toolbar ) : buildToolbarConfig() );
